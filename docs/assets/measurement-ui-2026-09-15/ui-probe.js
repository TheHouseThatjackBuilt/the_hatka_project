...
if (location.search.includes('error=1')) {
  ...
  window.fetch = (url, init) =>
    String(url).includes('model.json')
...
panel.style.cssText =
  'position:fixed;right:60px;top:35%;z-index:100;background:#20252a;color:white;padding:4px;max-width:360px;font:11px monospace';
...
function capture() {
  ...
  if (!state)
    return {
      ...
      portals: document.querySelectorAll('.hatka-select-popup').length,
      overlays: document.querySelectorAll('.ap-measurements').length,
      roots: _roots.size,
    };
  const { camera, scene, gl } = state;
  const mode = document.querySelector('.hatka-segmented-input:checked').value;
  const fitRect = document.querySelector('.viewer-fit-area').getBoundingClientRect();
  let minX = Infinity,
    maxX = -Infinity,
    minY = Infinity,
    maxY = -Infinity;
  ...
  return {
    ...
    fitRect: fitRect.toJSON(),
    fill:
      Math.max(
        ((maxX - minX) * viewport.clientWidth) / fitRect.width,
        ((maxY - minY) * viewport.clientHeight) / fitRect.height,
      ) / 2,
    center: [
      ((minX + maxX + 2) * viewport.clientWidth) / 4,
      ((2 - minY - maxY) * viewport.clientHeight) / 4,
    ],
